...
class Encoder(object):
    def __init__(self, model_pre_path):
        ...
        with tf.variable_scope('encoder'):
            self.weight_vars.append(self._create_variables(1, 4, 3, scope='conv1_1'))
            self.weight_vars.append(self._create_variables(4, 8, 3, scope='conv1_2'))
...
def conv2d(x, kernel, bias, use_relu=True):
    ...
    # conv and add bias
    #out = tf.nn.depthwise_conv2d(x_padded, kernel, strides=[1, 1, 1, 1], padding='VALID')
    out = tf.nn.conv2d(x_padded, kernel, strides=[1, 1, 1, 1], padding='VALID')
    out = tf.nn.bias_add(out, bias,name="bias_add")

    if use_relu:
        out = tf.nn.relu(out,name="conv2d_relu")
